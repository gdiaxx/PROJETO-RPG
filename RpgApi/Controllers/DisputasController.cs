...
[HttpPost("Habilidades")] //ATAQUE COM HABILIDADES
...
[HttpPost("DisputaEmGrupo")]
    public async Task<IActionResult> DisputaEmGrupo(Disputa d)
    {
        ...
        bool derrotado = false; //fazer looping ate o adv ser derrotado
        while (!derrotado)
        {
            foreach(Personagem atacante in personagens) //ter uma rodada de ataques
            {
                List<Personagem> oponentes = personagens.Where(p=> p.Id != atacante.Id).ToList(); //busca de oponentes excluindo o atacante
                ...
                if(ataqueUsaArma) //ataque com arma ou habilidade(else)
                {
                    //PROG DO ATAQUE COM ARMA
                ...
                if(oponente.PontosVida <=0)
                {
                    ...
                    string resultadoFinal = $"{oponente.Nome} foi derrotado! {atacante.Nome} ganhou com {atacante.PontosVida} restantes!";
            ...
            //fora do foreach: recomposicao dos pontos de vida, atualizacao do numero de lutas;
